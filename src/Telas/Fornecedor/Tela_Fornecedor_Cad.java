 ...
 * @author DIOGO e eric
 */
public class Tela_Fornecedor_Cad extends javax.swing.JInternalFrame {
    ...
    private void consultarFornecedor() {
        ...
        try {
            ...
            tblForn.setModel(DbUtils.resultSetToTableModel(resulset));
            procedAjustaTable();
        } catch (Exception e) {
//            JOptionPane.showMessageDialog(null, e);
    ...
    private void cadastarFornecedor() {
        // cadastra as informacoes do fornecedor no banco de dados
        String sql = "insert into tb_fornecedor(fornecedor,"
                + " CNPJ, CEP, telefone, UF, email,"
                + " endereco, obs, bairro, cidade)"
                + " values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                            ...
                            if (telEncontrado) {
                                if (cepEncontrado) {
                                    ...
                                    if (adicionado > 0) {
                                        ...
                                        if (rs.next()) {
                                            System.out.println("entrei no rs.next()");
                                            idForn = rs.getInt(1); //idForn gerado
                                            getCadastrarLogForn(3); //INSERT
                                            limpaFornecedor();
                                        }
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setResizable(true);
        setTitle("Tela Cadastro Fornecedor");
