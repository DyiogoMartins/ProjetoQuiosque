 ...
 * @author DIOGO eric
 */
public class Tela_Funcionario_Cad extends javax.swing.JInternalFrame {
    ...
    private void adicionar() {
        String sql = "INSERT INTO tb_funcionario("
                + " nomeFunc, Telefone, Email, login, RG, CPF, DataDeNasc,"
                + " Sexo, Cargo, Setor, Endereco, Bairro, Cidade,"
                + " CEP, UF, situacao, Obs, senha)"
                + " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,"
        ...
        try {
            ...
            // chama a funcao que troca / por -                
            pst.setString(7, funcaoDataNasc(txtDataUser.getText()));
            // chama a funcao que verifica o sexo informado                
            ...
            // chama a funcao que gera uma senha aleatoria 
            pst.setString(18, funcaoSenha());

            // isEmpty() é um método da classe String que verifica se uma
            //string está vazia.
            //método retorna um valor booleano true se a string estiver vazia
            if ((AtxtNome.getText().trim().isEmpty()
                    && AtxtNome.getText().trim().length() < 4)
                    ...
                    || (cboSituacao.getSelectedItem().toString().trim().isEmpty())) {
                ...
                //caso contrario, método retorna um valor booleano false
                //se a string nao estiver vazia
                if (emailEncontrado) {
                    if (cpfEncontrado) {
                        if (rgEncontrado) {
                                    ...
                                    if (cepEncontrado) {
                                        ...
                                        if (adicionado > 0) {
                                            ...
                                            if (rs.next()) {
                                                ...
                                                idFunc = rs.getInt(1); //idFunc gerado
                                                getCadastrarLogFun(3); // insert
                                                limpar();
                                            }
                                        ...
                                        int resposta = JOptionPane.showConfirmDialog(null,
                                                "CEP nao encontrado! Deseja continuar com o cadastro?", "Confirmação", JOptionPane.YES_NO_OPTION);
    ...
    private String funcaoSenha() {
        Verifica_Validacao funcoes = new Verifica_Validacao();
        String verificacao = funcoes.validaSenha();
        //System.out.println("verificacao: " + verificacao);
        return verificacao;
    }
    ...
    private void initComponents() {
        ...
        txt_Id1.setFont(new java.awt.Font("Arial", 0, 17)); // NOI18N
        txt_Id1.setEnabled(false);
        txt_Id1.setMinimumSize(new java.awt.Dimension(45, 45));
        txt_Id1.setPreferredSize(new java.awt.Dimension(45, 45));
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.gridx = 1;
        gridBagConstraints.gridy = 0;
        gridBagConstraints.fill = java.awt.GridBagConstraints.HORIZONTAL;
        ...
        gridBagConstraints.weighty = 1.0;
        gridBagConstraints.insets = new java.awt.Insets(5, 5, 5, 5);
        pnBaixo.add(txt_Id1, gridBagConstraints);
        ...
        cboUserCargo.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                cboUserCargoActionPerformed(evt);
            }
        });
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.gridx = 5;
    ...
    private void cboUserCargoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cboUserCargoActionPerformed
        // TODO add your handling code here:
    }//GEN-LAST:event_cboUserCargoActionPerformed
