 ...
 * @author diogo e eric
 */
public class Tela_Mesa_Alt extends javax.swing.JInternalFrame {
    ...
    private void setarCampos() {
        ...
        jspNumMesa.setValue(tblMesa.getModel().getValueAt(setar, 1));
        jspNumCap.setValue(tblMesa.getModel().getValueAt(setar, 2));
        AtxtObs.setText(tblMesa.getModel().getValueAt(setar, 4).toString());
        cbo_Stat.setSelectedItem(tblMesa.getModel().getValueAt(setar, 3).toString());
        txt_IdMesa.setText(tblMesa.getModel().getValueAt(setar, 0).toString());

    }

    private void alterar() {
        String sql = "update tb_Mesa set NumMesa=?, Capacidade=?,"
                + " status=?, obs=?"
                + " where idMesa = ?";
        ...
        try {
            ...
            pst.setString(5, txt_IdMesa.getText());
...
//                System.out.println(adicionado);
            ...
            JOptionPane.showMessageDialog(rootPane, "ERRO NO CATCH: " + e);
        }
    }
    ...
    private void remover() {
        ...
        if (confirmar == JOptionPane.YES_OPTION) {
            ...
            String sql = "delete from tb_mesa where idMesa = ?";
            try {
                pst = conexao.prepareStatement(sql);
                pst.setString(1, txt_IdMesa.getText());
                int apagado = pst.executeUpdate();

                if (apagado > 0) {

                    JOptionPane.showMessageDialog(rootPane, "Mesa removida com sucesso!");

                    getCadastrarLogMesa(3); // delete
                    limpar();
                    consultarMesa();
                } else {
                    JOptionPane.showMessageDialog(rootPane, "ID(Identificação) da mesa nao escolhida!", "Atenção!", HEIGHT);
                }
...
//                JOptionPane.showMessageDialog(null, e);
    ...
    private void initComponents() {
        ...
        jspNumCap.setModel(new javax.swing.SpinnerNumberModel(1, 0, null, 0));
        ...
        jspNumMesa.setModel(new javax.swing.SpinnerNumberModel(1, 0, null, 0));
